...
class ltBase
{
    ...
    {
        $type = $this->getPluginParameter('listtooltype');
        
        $inauthOK   = $this->in_auth->isObjectAllowed('listtool_cbox', $this->getOid());
        $hasAssignments = $this->in_auth->hasObjectAssignments('listtool_cbox', $this->getOid());
        
        if ($type == "backend" && $this->auth->isInPages($this->pid)) {
            return true;
        }
        if ($type == "intranet" && $inauthOK && $hasAssignments) {
            return true;
        }
        if (($type == "intranet_backend" || !$type) && (($inauthOK && $hasAssignments) || $this->auth->isInPages($this->pid))) {
            return true;
        }
        return false;
    }
